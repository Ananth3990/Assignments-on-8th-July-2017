...
GLfloat *translationMatrix1 = NULL;
GLfloat *identityMatrix = NULL;
GLfloat *scalingMatrix = NULL;
...
GLfloat* GetTranslationMatrix1()
{
	return (translationMatrix1);
}

void SetTranslationMatrix1(GLfloat **mat)
{
	translationMatrix1 = *mat;
}
...
void SetScalingMatrix(GLfloat **mat)
{
	scalingMatrix = *mat;
}

GLfloat* GetScalingMatrix()
{
...
void initialize(void)
{
	...
	TranslationMatrix(-1.5f, 0.0f, -7.0f, &translationMatrix);
	SetTranslationMatrix(&translationMatrix);

	//glTranslatef(1.5f, 0.0f, -6.0f);
	GLfloat *translationMatrix1 = NULL;
	AllocateMatrix(&translationMatrix1, NUMBER_OF_ROWS, NUMBER_OF_COLOUMNS);
	TranslationMatrix(1.5f, 0.0f, -7.0f, &translationMatrix1);
	SetTranslationMatrix1(&translationMatrix1);

	//glScalef(0.75f,0.75f,0.75f);
	GLfloat *scalingMatrix = NULL;
	AllocateMatrix(&scalingMatrix, NUMBER_OF_ROWS, NUMBER_OF_COLOUMNS);
	ScalingMatrix(0.75f, 0.75f, 0.75f, &scalingMatrix);
	SetScalingMatrix(&scalingMatrix);
...
void display(void)
{
	...
	glMultMatrixf(GetScalingMatrix());
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadMatrixf(GetIdentityMatrix());
	glMultMatrixf(GetTranslationMatrix1());
